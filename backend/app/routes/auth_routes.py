...
from ..extensions import get_login_identifier_key, limiter


auth_bp = Blueprint("auth", __name__, url_prefix="/api/auth")
MINIMUM_PASSWORD_LENGTH = 8
REGISTRATION_CONFLICT_ERROR = "Unable to create account with the provided details."
...
def register_customer():
    ...
    try:
        ...
        if username_exists:
            database.rollback()
            return jsonify({"error": REGISTRATION_CONFLICT_ERROR}), 400
...
@limiter.limit(
    ...
    key_func=get_login_identifier_key,
    methods=["POST"],
)
def login():
    data = valid_json()
    ...
    if not isinstance(login_name, str) or not login_name.strip() or not isinstance(password, str):
        return jsonify({"error": INVALID_LOGIN_ERROR}), 401
    database = get_db()
    try:
